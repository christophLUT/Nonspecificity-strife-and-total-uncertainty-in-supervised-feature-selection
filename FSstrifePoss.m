function [ranking,I]=FSstrifePoss(data,v,c,p)
% Function to calculate strife values according to possibility
...
% ranked third)
% I: information transmission values (strife values for each feature)
...
for k=1:l % classes
    for i=1:(sum(lc)) % samples in all classes
...
for k=1:l % classes
    for i=1:t % features
        ns1(i,k)=strifePoss(s1(1:lc(k),i,k)); % [subset samples; changed 23.07.2020]
        nsf1(i,k)=strifePoss(s1f(:,i,k));
    end
end
